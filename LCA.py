...
class LCA:
    ...
    Modified to allow for forest inputs by Robert Robere
    """
    def __init__(self, parent, RangeMinFactory = RestrictedRangeMin):
        ...
        for r in range(len(root)):
            levels = []
            self._replist.append({})
            self._visit(children,levels,root[r],0,r)
            self._rangeminlist.append(RangeMinFactory(levels))

    def __call__(self,*nodes):
        """Find least common ancestor of a set of nodes."""
        out = []
        r = []
        for i in range(len(self._replist)):
            for x in nodes:
                if x in self._replist[i]:
                    r.append(self._replist[i][x])
            if len(r) > 0:
                out.append(self._rangeminlist[i][min(r):max(r)+1][1])
        return out

    def _visit(self,children,levels,node,level, r):
        """Perform Euler traversal of tree."""
        self._replist[r][node] = len(levels)
        pair = (level,node)
        levels.append(pair)
        for child in children[node]:
            self._visit(children,levels,child,level+1,r)
            levels.append(pair)
